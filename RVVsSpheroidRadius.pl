...
print $gnuPlot "set title offset 0,-1 'Reddening at inclination \$i=".sprintf("%4.1f",$inclination->(($options{'inclinationIndex'})))."^\\circ\$ and face-on optical depth \$\\tau_\\mathrm{V,0}=".sprintf("%6.2f",$opticalDepth->(($options{'opticalDepthIndex'})))."\$'\n";
