...
my $yMinimum = $extinction->(:,($options{'inclinationIndex'}),:)->flat()->minimum();
my $yMaximum = $extinction->(:,($options{'inclinationIndex'}),:)->flat()->maximum();
...
for(my $i=0;$i<nelem($opticalDepth);++$i) {
	...
	\$plot,
	$wavelength,
	$extinction->(($i),($options{'inclinationIndex'}),:),
	style      => "line",
	weight     => [2,1],
	color      => [$color,$color]
	);
